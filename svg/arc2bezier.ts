...
function mapToEllipse (p: { x: number, y: number }, rx: number, ry: number, cosphi: number, sinphi: number, centerx: number, centery: number) {
    let x = p.x
    let y = p.y
...
function arcToBezier (
    ...
    cx: number, cy: number,
    rx: number, ry: number,
    xAxisRotation = 0, largeArcFlag = 0, sweepFlag = 0
) {
    const curves = []
